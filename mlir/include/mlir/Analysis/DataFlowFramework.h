...
struct LatticeAnchor
  ...
  using ParentTy =
      PointerUnion<GenericLatticeAnchor *, Operation *, Value, Block *>;
  ...
  LatticeAnchor(OpT op) : ParentTy(op) {}
...
public:
  ...
  const StateT *lookupState(AnchorT anchor) const {
    auto it = analysisStates.find({LatticeAnchor(anchor), TypeID::get<StateT>()});
...
private:
  ...
  /// The storage uniquer instance that owns the memory of the allocated lattice anchors
...
namespace llvm {
/// Allow hashing of lattice anchors.
template <>
